...
public:
  ...
  void readFile(vector<vector<string>> &lines, vector<OrderEntry> &orderEntries,
    ...
    while (getline(file, line)) {
      ...
      if (result[0] == 0) {
        if (result[1] != 0) {
          OrderEntry orderEntry(orderID, tokens[0], tokens[1], stoi(tokens[2]),
                                1, stod(tokens[3]), stoi(tokens[4]), result[1]);
  ...
  void writeFile(vector<OrderEntry> &orderEntries) {
    ...
    file << "OrderID,ClientOrderID,Instrument,Side,ExecStatus,Price,Quantity,"
            "Reason\n";

    for (auto &orderEntry : orderEntries) {
      file << orderEntry.getOrderID() << "," << orderEntry.getClientOrderId()
...
int main() {
  ...
  vector<vector<string>> validOrders;
